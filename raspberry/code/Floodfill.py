...
class Floodfill:
    ...
    def __init__(self, fieldNr = 21, driveableThreshold = 1):
    ...
    def fill(self, startPos):
        ...
        pixel = self.fields[5, 15]
        pixel[0] = self.waypointColor[0]
        pixel[1] = self.waypointColor[1]
        pixel[2] = self.waypointColor[2]
        self.fields[5, 15] = pixel
        ...
        while len(stack) > 0:
            ...
            if x > 0 and x < self.fieldNr and y > 0 and y < self.fieldNr:
                ...
                if driveable and not destinationFound:
                    ...
                    #print(lastValue)
                    if newValue < value:#git pull
                        # and value == self.fieldNr * self.fieldNr:
                        lastValue = newValue
                        pixel[0] = (int)(newValue * 127 / (self.fieldNr * self.fieldNr)) + 127
                        self.fields[y, x] = pixel
                        values[x, y] = newValue
